...
for NUM_DIM_MI = 1:5:size(images_data,2)-2
    images_data = [apply_mi(images_data_prev(:, 2:end-1),NUM_DIM_MI) images_data_prev(:, end)];
    ...
    for k = 1:length(files)
        ...
        if (length(file) > length(ends_with) && strcmp(file(end - length(ends_with) + 1 : end), ends_with) == 1)
            ...
            f = fopen('new_mutualinfo.csv', 'a');
            fprintf(f, '%d', NUM_DIM_MI);
